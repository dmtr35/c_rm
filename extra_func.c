...
char *replace_slashes_dash(char *file_path, size_t length_dir_file)
{
    ...
    strncpy(slash_or_dash, file_path, 1);
    // size_t length_symbol = sizeof(slash_or_dash) / sizeof(*slash_or_dash);
    // printf("slash_or_dash: %s\n", slash_or_dash);
    // printf("length_symbol: %zu\n", length_symbol);
    // printf("slash_or_dash: %s\n", slash_or_dash);
    ...
    for (int i = 0, j = 0; i < length_dir_file; ++i) {
        if (file_path[i] == first_symbol[0]) {
            result[j++] = second_symbol[0];
        } else {
            result[j++] = file_path[i];
        }
    }
...
void extractFileNameAndPath(const char *arg_token, char *file_name, char *dir_file)
{
    ...
    char *res = replace_slashes_dash(dir_file, length_dir_file);
    // printf("res:2: %s\n", res);
